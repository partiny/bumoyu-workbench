...
function handleSendCode() {
  ...
  http.post(ApiAuth.sendEmailCodeOfResetPassword, { email: form.email })
    .then(res => {
...
function handleSubmit() {
  formRef.value.validate().then(() => {
    ...
    loading.value = true
    http.post(ApiAuth.resetPassword, params)
      .then(res => {
        loading.value = false
        const { message, success } = res || {}
        if (!success) {
          toast.error(message || '密码重置请求错误')
          return
        }
